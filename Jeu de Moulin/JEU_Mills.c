...
{ // le traitement a effectuer si le double moulin est verifie:
  ...
  if (joueur==1) {adv=2;score_adv=score(2);}
    ...
    if(((score_adv>3)&&(Somme_Etat(adv)==score_adv))||(score_adv<=3))
    {
     printf("JOUEUR %d :DOUBLE MOULIN : vous avez le droit de capture \n\n",joueur);
     color(15,0);
  ...
  {  // permet de verifier si un double moulin existe
     ...
     {if((ETAT[d]==0)&&(ETAT[c]==0)&&(ETAT[b]==0)&&(ETAT[a]==0))
                    return 1; //la condition est vÈrifiÈ
      else           return 0;
     }
   else                         return 0;
  }
...
//LE NOMBRE DE CASE NON BLOQUEES POUR CHAQUE JOUEUR : TEST DE FN FU JEU A CAUSE DU BLOCAGE
{
 int comp=0;
...
int i,a,x;
 printf("Le JOUEUR %d\n",joueur);
do{printf("\t\t l'emplacement a deplacer :\n");
      scanf("%d",&i);
...
int test_fin(int joueur,int adv)
 {
     if(score(joueur)<=5)return 1;
       else if (blocage(joueur)==0) return 1;
     else return 0;
 }
...
while((test_fin(LAS,DOS)==0)&&(test_fin(DOS,LAS)==0))
 ...
 if(test_fin(LAS,DOS)==1)
     {congratus(DOS);
         Gagnat=DOS;
         RANKING1();}

if(test_fin(LAS,DOS)==1){congratus(LAS);
         Gagnat=LAS;
         RANKING1();}}
...
void lost()
{  system("PAUSE");
system("cls"); printf("\tUNFORTUNATELY :\n\a");
    color(4,0);
printf("  \t\t\t\t\t\tYY      YY OOOOOOOOOO  UU     UU\n");color(11,0);
printf("  \t\t\t\t\t\t YY   YY   OO      OO  UU     UU\n");color(2,0);
printf("  \t\t\t\t\t\t  YY YY    OO      OO  UU     UU\n");color(3,0);
...
int test_fin_M()
 {
     if((score(1)<=5)||(blocage(1)==0))return 2;

     else if((score(2)<=5)||(blocage(2)==0))return 1;
...
grille();getchar();system("cls");srand(time(NULL));
 LAS=1+rand()%2;  //LE PREMIERE A JOUER
 printf("%d\n",LAS);
for(int i=0;i<9;i++){ //PHASE REMPLISSAGE
if (LAS ==1) {remplissage_joueur(1);
             TRAITEMENT(1);
             remplissage_machine();
             TRAITMENT_ORDINATEUR();grille();}
if(LAS==2){  remplissage_machine();
             TRAITMENT_ORDINATEUR();
             remplissage_joueur(1);
             TRAITEMENT(1);grille();}//LE JOUEUR EST POUR NUMERO 1
getchar();system("cls"); }grille();
while(test_fin_M()==0)   //PHASE DEPLACEMENT
{  if (LAS==1){if (score(1)>3)deplacement(1);
                else VOLE(1);
                TRAITEMENT(1);grille();
                if(score(2)>3) deplacement_machine();
                else VOLE_MACHINE();
                TRAITMENT_ORDINATEUR();grille();}
    else        {if(score(2)>3) deplacement_machine();
              else VOLE_MACHINE();
              TRAITMENT_ORDINATEUR();grille();
             ...
             TRAITEMENT(1);grille();}}
if(test_fin_M()==2)
     {if(blocage(1)==0) {printf("\n VOUS NE POUVEZ PLUS BOUGER \n " );lost();} else {lost();}
     Gagnat=2;RANKING1();}
 if(test_fin_M()==1)
     {congratus(1);Gagnat=1;RANKING1();}}


//3EME PARTIE AI NIVEAU MOYENN
   int verify(int a,int b,int c,int nbr)

 // RECHERCHE DE POSSIBILTE DE CONSTITUTION DUN MOULIN OU BLOCAGE DUN MOULIN DE L'AUTRE
{
